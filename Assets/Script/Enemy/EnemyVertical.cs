...
public class EnemyVertical : EnemyController
{
    // Rigidbody2D En; // 적
    // SpriteRenderer SR;
    // Animator animation;
    ...
    void Awake()
    {
        ...
        //Think();
        //Invoke("Think", 5);
        //SR.flipY = h == 1; // 렌더링 방향 이동에 맞춰서 바꾸기
    ...
    void FixedUpdate()
    {
        ...
        // 이동
        En.velocity = new Vector2(En.velocity.x, v * speed);
        //Debug.Log(h);

        // 하단 이동 시 바닥 감지
        if (En.velocity.y < 0)
        {
            Debug.DrawRay(En.position, Vector2.down, new Color(0, 1, 0));
            RaycastHit2D rayHit = Physics2D.Raycast(En.position, Vector2.down, 1, LayerMask.GetMask("normalfloor", "blockedfloor"));
        ...
        {
            Debug.DrawRay(En.position, Vector2.up, new Color(0, 1, 0));
            RaycastHit2D rayHit = Physics2D.Raycast(En.position, Vector2.up, 1, LayerMask.GetMask("normalfloor", "blockedfloor"));
            //Debug.Log(rayHit.collider);
            if (rayHit.collider != null)
            {
                Turn();
