...
public class PlayerMoving : DamageController
{
    ...
    //public Slider CircleSlider; // circle skill 슬라이더 //1215 from jeongik
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.C))//1222 임시작업 1.0 만들시 삭제
        {
            CircleController.circleCount = 4;
            Circle[0].SetActive(true);
            
        }

        // 플레이어 데미지 1~1.5배 사이 지정 //1214from jeongik
        TotalPlayerDamage = PlayerAtkDmg * Random.Range(1.0f, 1.5f);//1214from jeongik
        ...
        if ((bottomjump && !anim.GetBool("jumping")) || (rigid.velocity.y< -0.0001f && !anim.GetBool("jumping")))
        {
            ...
            {
                anim.SetBool("jumping", true);
                //audio.PlaySound("JUMP"); // 1221 from jeongik
                jumpcnt += 1;
                //Debug.Log(jumpcnt);
        ...
        {
            gameManger.UIPanel.SetActive(true);
            gameManger.UIPanel.transform.GetChild(0).gameObject.SetActive(true);
            gameManger.UIPanel.transform.GetChild(0).GetChild(2).gameObject.SetActive(true);
            playerPortal.PortalButton();  // need Fix 1217                     
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (rayLeftHit.collider != null)
        {
            ...
            if (gameManger.isTalkPanelActive == true || gameManger.isStoreActive == true)
            {
                gameManger.SearchPanel.SetActive(false);
            }
            else
            {
                gameManger.SearchPanel.SetActive(true);
            }
        }
...
void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            audio.PlaySound("PickItem"); //1221 from jeongik
        ...
        {
            audio.PlaySound("POTION"); //1221 from jeongik
